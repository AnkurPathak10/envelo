...
- Feature 09 real-device follow-up: initial chat positioning and Android keyboard:
  ...
  - Replaced the chat's built-in `KeyboardAvoidingView` with `KeyboardStickyView` around only the composer. The message list keeps its normal layout while the composer follows the keyboard animation and retains its 8-point separation, avoiding both the earlier double-resize and native-resize-not-applied cases.
