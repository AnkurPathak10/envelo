...
- Chat bubble timestamp/receipt alignment — implemented; real-device visual acceptance pending.
  - Plain-text and caption metadata now occupies a reserved inline width while the visible timestamp and receipt stay anchored to the bubble's lower-right corner. Short messages remain compact and wrapped messages can no longer pull the metadata leftward or lift the receipt above the baseline.
