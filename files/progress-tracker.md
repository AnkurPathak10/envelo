...
- Feature 19: Floating Rich Message Composer — Milestone 1 implemented; typed rich-message milestones remain in progress.
  ...
  - CodeRabbit reply-safety follow-up: a missing reply target now returns a typed terminal acknowledgement. The durable queue removes only that stale reply reference and retries the unchanged text/media once as an ordinary message with the same client ID, preventing one invalid target from permanently blocking the conversation queue. Replyable bubbles also expose a bounded, screen-reader-only Reply button while retaining their existing visual swipe and nested controls.
  - Clear-history reply privacy follow-up: history and live socket payloads now evaluate quoted previews against each viewing participant's own `clearedAt`. A later reply cannot reveal the content or media of a target that account previously cleared, while another participant whose cutoff still permits it continues receiving the preview.
  - CodeRabbit follow-up verification passed: mobile, backend, and socket-server TypeScript; Expo lint; changed-file Prettier; and repository whitespace checks.
