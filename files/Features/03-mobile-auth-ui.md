...
- Browser auth requests send `X-Envelo-Platform: web` with credentials included. Login/signup set the scoped refresh cookie, refresh rotates it, and explicit logout clears it. A rejected refresh must not emit an expired `Set-Cookie`, because a slower stale 401 can arrive after another request has installed a newer rotated cookie. Browser JSON responses must not expose the raw refresh token; native clients retain the existing response-body/SecureStore contract.
- Refresh rotation uses an atomic compare-and-set revoke so only one concurrent request can consume a token. The shared API client also coalesces same-runtime refresh attempts into one in-flight promise; callers reuse its result before retrying their original request.
