...
- Clear chat and Delete chat always show confirmations. They are account-scoped participant actions: the result follows the signed-in account across mobile, localhost, and any other device, but never changes the other participant's copy or deletes shared `Message`/`Conversation` rows.
- `ConversationParticipant.clearedAt` is the authoritative history cutoff. Clear advances only the caller's cutoff, restores a previously hidden inbox row, removes that account's local cache/offline queue on each active client, and returns only messages newer than the cutoff from history, search, pagination, inbox preview, and unread-count queries.
- Reply previews obey the same per-participant cutoff. History mapping and live socket delivery omit a nested quoted target when that target predates the viewing account's `clearedAt`, preventing a later reply from revealing content that account already cleared while leaving the other participant's still-visible preview unchanged.
- `ConversationParticipant.deletedAt` is the authoritative inbox-deletion marker. Delete advances both participant timestamps and hides the row for that account until a later message exists. Selecting the same person in New conversation restores that participant's existing direct conversation without restoring cleared history, so the user can start chatting again instead of landing on an empty dead end.
- Clear/Delete are committed through an acknowledged, authenticated Socket.IO mutation whenever the live channel is available. The socket server updates only the caller's `ConversationParticipant` row, broadcasts the resulting server timestamp to every active session for that account, and returns success only after the database write completes. The authenticated REST DELETE handlers remain the fallback when the socket is disconnected; the shared conversation and messages remain intact for the other participant.
- A client must not hide messages or remove an inbox row merely because a local cache operation succeeded. It applies the UI change only after either the socket acknowledgement or REST response contains the authoritative participant timestamp. REST refresh remains the durable reconciliation path after reconnect/reload.
- Bubble content reserves explicit right-side width before its bottom-right timestamp/receipt. This leaves a deliberate visual gap after both sent and received text, while retaining the compact single-line layout and the timestamp/receipt's lower-right anchor.
- Verification must use account A on two clients plus account B on a third client: Clear/Delete on A1 must update A2, must not change B, and creating/opening the conversation again on A must succeed with only post-cutoff history.
